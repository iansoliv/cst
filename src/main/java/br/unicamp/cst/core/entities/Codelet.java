...
public abstract class Codelet implements Runnable {
	 ...
	 */
	protected List<Memory> inputs = new ArrayList<>();
	/**
	 * Output memories, the ones that are written.
	 */
	protected List<Memory> outputs = new ArrayList<>();
	/**
	 * Input memories, the ones that were broadcasted.
	 */
	protected List<Memory> broadcast = new ArrayList<>();
	 ...
	 * abstract method must be implemented by the user. 
	 * 
	 * Here, the user must get the inputs and outputs that 
	 * this codelet needs to perform its activity implemented in
	 * the proc() method.
	 */
	 ...
	 * 
	 * Here, the user must calculate the activation of 
	 * the codelet before it does what it is
	 ...
	 * This codelet executes this method as its main activity.
	 */
	 ...
	 *
	 * @param accesing the Codelet accessing.
	 ...
	 *
	 * @param accesing the Memory Buffer accessing.
	 ...
	 *
	 * @return the loop boolean flag current status.
	 */
	 ...
	 * @throws CodeletActivationBoundsException exception triggered if an
	 * activation lower than 0 or bigger than 1 is set
	 */
	 ...
	 *
	 * @param type the type of memories to be fetched from the output.
	...
	{
		ArrayList<Memory> selectedMemories = new ArrayList<>();

		if (outputs != null && outputs.size() > 0)
		{
			this.outputs.stream()
					.filter( (mo) -> (mo.getName() != null && 
							      mo.getName().equalsIgnoreCase( type )) )
					.forEachOrdered( (mo) ->	
					{ 
						selectedMemories.add( mo ); 
					} );
		}
	 ...
	 *
	 * @param b one broadcast input to set.
	 */
	...
	{
		Memory inputMO = null;
		ArrayList<Memory> listMO = new ArrayList<>();
	 ...
	 *
	 * @param name the name of the memory being searched.
	...
	public synchronized Memory getOutput( String type, int index )
	{
		...
		if (outputs != null && outputs.size() > 0)
		{
			listMO = outputs.stream()
						.filter( (mo) -> (mo != null && 
									type != null && 
									mo.getName() != null && 
									mo.getName().equalsIgnoreCase( type )) )
						.collect( Collectors.toList() );		// return an empty List in case the filter result is null.
	 ...
	 * @param index the index to be considered while multiple equal names are
	 ...
	 * @throws CodeletThresholdBoundsException the exception thrown if the
	 * threshold value is less than zero or greater than 1.
	 */
